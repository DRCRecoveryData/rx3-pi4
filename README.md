...
|---|---|---|
| Invocation | `bash ~/rx3-pi4-install.sh` (wrong path) | `cd ~/rx3-pi4 && bash rx3-pi4-install.sh` |
| Auto-start on boot | Manual | `sudo systemctl enable rx3.service` |
| Auto-reboot | None | 10 s countdown, then `sudo systemctl reboot` |
| Skip reboot | N/A | `AUTO_REBOOT=0 bash rx3-pi4-install.sh` |
| Prerequisite check | Silent | Prints host, user, fb, reboot status |
| Sudo prompt | Scattered | One prompt up front, then unattended |
| README | Basic | Full setup + troubleshooting |

### 🚀 Run It

```bash
cd ~/rx3-pi4
# overwrite the old script with the new one above
nano rx3-pi4-install.sh      # paste the new content
chmod +x rx3-pi4-install.sh
bash rx3-pi4-install.sh 2>&1 | tee ~/rx3-install.log
```

After the reboot (automatic), the DSI panel shows the RX3 UI within ~60 s. Check with:
